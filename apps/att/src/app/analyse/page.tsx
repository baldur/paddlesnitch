...
function PaddleCard({ p }: { p: Paddle }) {
  ...
  return (
    <Link href={`/analyse/${p.id}`} className="border border-[#1e293b] rounded p-3 flex gap-3 items-center hover:border-[#0369a1] transition-colors">
    ...
    <Frame nav={<Link href="/analyse/library" className="text-muted hover:text-fg tracking-widest transition-colors">MY PADDLES</Link>}>
      <div className="max-w-3xl mx-auto px-4 py-6 w-full">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link href="/analyse/new" className="px-4 py-2 bg-[#0369a1] text-white text-xs font-bold tracking-widest rounded hover:bg-[#0284c7]">+ ANALYSE A PADDLE</Link>
        </div>
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {cards.length > RECENT && (
            <Link href="/analyse/library" className="text-[11px] tracking-widest text-[#64748b] hover:text-[#e2e8f0]">VIEW ALL {cards.length} →</Link>
          )}
        </div>
        ...
        {!loading && cards.length === 0 && (
          <div className={`${PANEL} p-6 text-center`}>
            ...
            <Link href="/analyse/new" className="mt-3 inline-block px-5 py-2.5 bg-[#0369a1] text-white text-xs font-bold tracking-widest rounded hover:bg-[#0284c7]">ANALYSE YOUR FIRST ONE →</Link>
          </div>
        )}
