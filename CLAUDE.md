...
- [`share-image-strava.md`](docs/features/share-image-strava.md) — 🚧 spec 2026-08-23 (#212, not built). Add a branded **share-card OG image** to the opt-in public paddle link (#202) so it unfurls on social + is downloadable, and a **no-scope Strava helper** in the SHARE panel (deep-link to the activity + copy link + download image; user pastes/attaches manually). Read-only — **no `activity:write`**. The card carries a **QR to the paddle's public share URL** (chosen 2026-09-06) — the only in-Strava acquisition path, since the feed doesn't unfurl the link. Hard Strava limits recorded: the API can't attach photos to an activity, and Strava's feed doesn't unfurl link previews. OG image via Next's `opengraph-image` route convention (the shared page is a client component, so meta must come from the route segment). **P1 shipped** (card + QR + DOWNLOAD IMAGE); P2 Strava helper + P3 (deferred) activity:write auto-append remain.
...
- **Share a paddle (#202)**: an owner can opt a saved paddle into an **unlisted public link** and revoke it. `analysis-store.ts` `shareSession`/`unshareSession`/`getSharedSession` maintain a share index `analysis/shared/{shareId}.json → { userId, sessionId }` (so the token resolves to the owner's session without the userId appearing in the URL; idempotent — same link on repeat share; `deleteSession` drops the index; a stale index whose session no longer bears the token doesn't resolve). Owner routes `POST`/`DELETE /analyse/api/analyse/sessions/[id]/share`; public `GET /analyse/api/analyse/shared/[shareId]` (strips the private diary note — exposes result + narrative + boat only). The public page `/analyse/shared/[shareId]` renders `AnalysisView` in `readOnly` mode (no edit controls; "analyse your own" CTA). `AnalysisView` shows a `SHARE` control (owner only). No `AnalysisSession` schema change beyond an optional `shareId`.
  - **Share-card image (#212, P1):** the shared route has an `opengraph-image` (`app/shared/[shareId]/opengraph-image.tsx`, `next/og`) — a branded 1200×630 card with the route polyline + distance/time/pace/rate + boat tag + wordmark + a **QR to the paddle's public URL** (so a viewer who sees the card as a Strava photo — Strava doesn't unfurl the link — can scan through to the app). Pure data via `shareCard()` (`src/lib/share-card.ts`, tested); QR via `qrcode` at render time; missing/revoked link → a generic branded fallback (never leaks). The SHARE panel has a **DOWNLOAD IMAGE** button (fetches the OG image → saves it) so the owner can add it as a Strava photo or post it. See [`share-image-strava.md`](docs/features/share-image-strava.md).
