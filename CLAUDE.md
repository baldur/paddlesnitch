...
- [`single-app-shell.md`](docs/features/single-app-shell.md) — 🚧 built, in review 2026-08-09 (**P1–P4 done, P5 core done**; platform now DARK). Make att + Analyse feel like **one app**: a new **`packages/ui`** shared shell (theme-aware, presentational/config-driven) with `AppShell`/`AppHeader` (breadcrumb + cross-app Trials↔Analyse nav + `AccountNav` + feedback), a unified `ContactBanner`, and **ONE `FeedbackWidget`** (collapsing att's `FeedbackTrigger` + Analyse's widget; both POST `/att/api/feedback`). Shared strava-email helpers move to `@paddlesnitch/core`. Plus **theme convergence to dark** platform-wide (extend Analyse's palette): att already has `@theme` tokens but ~32/45 tsx use hardcoded hex → migrate both apps' components to semantic token classes, then flip token values to dark; att maps default to dark tiles. Phased P1 scaffold ui+tokens+widget (adopt in Analyse first) → P2 adopt shell in att → P3 att hex→token migration (page-group by page-group, still light) → P4 flip to dark → P5 cross-app nav + unified profile. Chrome/styling only; no data/auth/API change.
...
- [`device-uplink.md`](docs/features/device-uplink.md) + [`device-data.md`](docs/features/device-data.md) — ✅ shipped 2026-09 (#214/#215/#216). Hardware tracker (LilyGO T-Beam S3 Supreme) claims + binds to an account (TV-style code pairing), uploads raw CSV sessions (reusing `parseTrace` unchanged), which surface as a **MY TRACKER** paddle source in Analyse; a **device data page** (`/profile/me/devices`) + `describeDeviceData` diagnostics list every column with movement-gated distance and an honest stroke-rate verdict. See the **Devices** subsection under Ops. Stroke rate is not derivable from firmware 0.3.0 (per-second 1 Hz accel peak) — needs device-side cadence.
- [`share-image-strava.md`](docs/features/share-image-strava.md) — 🚧 spec 2026-08-23 (#212, not built). Add a branded **share-card OG image** to the opt-in public paddle link (#202) so it unfurls on social + is downloadable, and a **no-scope Strava helper** in the SHARE panel (deep-link to the activity + copy link + download image; user pastes/attaches manually). Read-only — **no `activity:write`**. Hard Strava limits recorded: the API can't attach photos to an activity, and its feed doesn't unfurl link previews (but it **linkifies** URLs, so a pasted link is tappable). **No QR** (tried then dropped — needs a second device's camera, useless phone-on-phone; the tappable pasted link is the mobile path). OG image via Next's `opengraph-image` route convention (the shared page is a client component, so meta must come from the route segment). **P1 + P2 shipped** (card + DOWNLOAD IMAGE + Strava deep-link/paste nudge); P3 (deferred) activity:write auto-append remains.
- [`behavioural-analytics.md`](docs/features/behavioural-analytics.md) — 🚧 spec 2026-08-09 (not built). Capture **semantic struggle signals** (rage/dead/error clicks, pogo-sticking, scroll thrash, form struggle, long dwell — NO raw coordinates, NO session replay), **consent-gated (opt-in banner)**, custom in-stack: client `capture()` buffer → `POST /att/api/behaviour` → S3 JSONL per session → **nightly scheduled Lambda** reduces each session to a compact semantic timeline and batches ~20–30 per **Bedrock Haiku/Nova** call to label + cluster confusion patterns (LLM interprets, deterministic heuristics detect). Off the request path; retention-capped; anonymous `sid` only, input values never recorded. Phased P1 consent+capture+ingest+storage → P2 reducer+LLM → P3 clustering+report → P4 wire Analyse → P5 tune signals.

- [`sitemap.md`](docs/features/sitemap.md) — 🚧 proposal 2026-09-06 (not executed). Current-state sitemap (every route across platform / ATT / Analyse, annotated with audience + auth) + a proposed IA reorg for discussion: a real front door at `/`, keep platform concepts platform-level (propose `/att/groups`→`/groups`), deeper cross-app links, and the `/att`-rename presented as a deferred option with its cost. Route moves to be done later in small redirected steps (mirroring `profile-routes.md`).
...
strava/callback       GET  — verifies state, exchanges code, persists tokens, redirects to /profile/me/settings?strava=connected
...
The shared UI shell that gives both apps a single-app feel (🚧 in progress — see
[`single-app-shell.md`](docs/features/single-app-shell.md)). Theme-aware,
**presentational + config-driven** (props: user, hrefs, callbacks, endpoint — no
`/att` vs `/analyse` routing baked in), per-file subpath exports, `transpilePackages`d.
...
src/
  ...
  components/
    AuthNav.tsx                ← Client component: signed in → name (links to /profile/{id} public profile) + ACCOUNT (links to /profile/me/settings settings) + SIGN OUT; signed out → "SIGN IN" link
...
Profile + account are **platform-level routes** ([`profile-routes.md`](docs/features/profile-routes.md)): public profile `/profile/{id-or-handle}`, your own `/profile/me` (→ redirects to your public profile, which shows a SETTINGS link when it's yours), account settings `/profile/me/settings`, account API `/api/account/*`. Old `/att/u/:id` + `/att/account` 301-redirect to the new URLs (next.config). `/profile/me*` + `/api/account` mutations are auth-gated in `src/proxy.ts`; `/profile/:id` is public.
...
- **Share a paddle (#202)**: an owner can opt a saved paddle into an **unlisted public link** and revoke it. `analysis-store.ts` `shareSession`/`unshareSession`/`getSharedSession` maintain a share index `analysis/shared/{shareId}.json → { userId, sessionId }` (so the token resolves to the owner's session without the userId appearing in the URL; idempotent — same link on repeat share; `deleteSession` drops the index; a stale index whose session no longer bears the token doesn't resolve). Owner routes `POST`/`DELETE /analyse/api/analyse/sessions/[id]/share`; public `GET /analyse/api/analyse/shared/[shareId]` (strips the private diary note — exposes result + narrative + boat only). The public page `/analyse/shared/[shareId]` renders `AnalysisView` in `readOnly` mode (no edit controls; "analyse your own" CTA). `AnalysisView` shows a `SHARE` control (owner only). No `AnalysisSession` schema change beyond an optional `shareId`.
  - **Share-card image + Strava helper (#212, P1+P2):** the shared route has an `opengraph-image` (`app/shared/[shareId]/opengraph-image.tsx`, `next/og`) — a branded 1200×630 card with the route polyline + distance/time/pace/rate + boat tag + wordmark + `paddlesnitch.com`. Pure data via `shareCard()` (`src/lib/share-card.ts`, tested); missing/revoked link → a generic branded fallback (never leaks); sets `og:image` (X uses it as the twitter fallback — no `twitter-image`, which Turbopack can't statically read as a re-export). **No QR** — it needs a second device's camera, useless phone-on-phone; the click-through is the **tappable link** the owner pastes (Strava linkifies URLs) + the OG unfurl on social. SHARE panel: **DOWNLOAD IMAGE** (fetch the OG image → save) for all paddles, and for Strava-sourced ones an **OPEN MY STRAVA ACTIVITY ↗** deep-link (needs `stravaActivityId` on the owner's `ViewData.source`) + a paste-the-tappable-link nudge. See [`share-image-strava.md`](docs/features/share-image-strava.md).
- **Routes** (under `/analyse/api/`): `analyse` (main pipeline), `analyse/sessions[/[id]][/share]`, `analyse/shared/[shareId]` (public), `analyse/similar[/compare]`, `analyse/section-insight`, `trials`, `strava/activities`, `me`.
...
- **Client events (buffered + batched, BOTH apps):** the capture lives in **`@paddlesnitch/ui/analytics`** (`capture(event, props?)`), and both apps mount the shared **`@paddlesnitch/ui/Analytics`** component in their root layout (att via a re-export shim at `@/components/Analytics`; analyse imports it directly). `capture()` appends to an in-memory queue and **flushes a batch** to `POST /att/api/track` on whichever comes first: a timer (`flushIntervalMs`, 15 s), the queue hitting `maxBatch` (20), or the tab being hidden/closed (`visibilitychange→hidden` / `pagehide`, via `navigator.sendBeacon`). The endpoint is an absolute path so it reaches the att server from **both** apps across the shared CloudFront origin (fetch/sendBeacon ignore Analyse's `basePath`). The `Analytics` component records a `pageview` (with `window.location.pathname`, the true full path incl. `/analyse`) on each route change. **On in production builds**, off in dev/test; `NEXT_PUBLIC_ANALYTICS=0` is the kill switch (tuning knobs on `_config`). No PII: only event, path, small string props, and a random per-tab `sid`. Vocabulary is a **strict allowlist** — the single source of truth is **`@paddlesnitch/ui/metrics-events`** (`METRIC_EVENTS`), re-exported by att's `src/lib/metrics.ts` (which keeps the server EMF `buildEmf`/`emitMetric`); enforced client-side (dropped before queueing) AND server-side. To add an event, add it to `METRIC_EVENTS`, then `capture('your_event')`. att keeps a re-export shim at `@/lib/analytics`.
...
- **Pairing (TV-style):** device `POST /api/devices/claim` → shows a 6-char code on its OLED; user enters it at `/profile/me/settings` → Devices (`POST /api/account/devices/link`); device polls `POST /api/devices/token` and collects a per-device token. Secrets + tokens stored only as `sha256`; claims single-use + 10-min TTL; `deviceId` (efuse-MAC hex) is **not** a secret. Manage/revoke via `GET`/`DELETE /api/account/devices`.
- **Upload:** `POST /api/devices/sessions?filename=…` (device Bearer), raw CSV (≤2 MB), idempotent by `deviceId`+`filename`. **Reuses `parseTrace()` UNCHANGED** — the firmware conforms to the existing CSV columns, so the device's `lat`/`lon`/`timestamp` need **no** device-specific parser. Column names are therefore **load-bearing**; unfixed rows write empty `lat`/`lon` (never `0,0` — see Null Island in the spec) and yield `422` (a bench log, not an error).
- **As a paddle source:** device sessions appear under **MY TRACKER** in Analyse (`/analyse/api/devices`; `source.type: 'device'`), analysed on demand like a trial entry.
- **Diagnostics:** `describeDeviceData(csv)` (`@paddlesnitch/timing/device`) reports every column, fix/no-fix counts, **movement-gated** distance (anchor-based ≥3 m at ≥1.5 km/h — never a raw fix sum; GNSS scatter otherwise invents ~115 m), and an **honest stroke-rate verdict**. The **device data page** (`/profile/me/devices`) surfaces all of it. **Stroke rate is NOT derivable from firmware 0.3.0** — it logs a per-second accel *peak* at 1 Hz, which can't yield a 0.5–2 Hz cadence; the fix is device-side cadence emitting a `strokerate` column (already handled by `parseCsv`, so no server change). Do not fake it from `accel_mag_max_g`.
...
- **Sidecar transport:** `POST /api/devices/sessions?filename=…_imu.csv` routes **past `parseTrace`** (a sidecar carries no position) and attaches to the already-uploaded track of the same name via `storeDeviceMotion`; stored at `devices/{deviceId}/sessions/{sessionId}/motion.csv`. **409 = the track isn't up yet → the device retries** (firmware uploads sidecars in a SECOND pass after every track, and must not mark a 409 sidecar done). `MAX_BYTES` raised 2→4 MB; a Lambda function URL caps near 6 MB, so it can be raised but not removed. The full 50 Hz file stays on the card and is never auto-deleted.
- **Deferred:** rate limits (need a real store/WAF), attaching a device session to a trial submission.
