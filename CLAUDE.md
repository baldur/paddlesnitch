...
- **OTA + device auth:** [`device-ota-and-auth.md`](docs/features/device-ota-and-auth.md) — Phase 0 (repartition) **shipped** #256; phases 1–4 (manifest + presigned URL, observability, firmware, release workflow) are spec only. **A partition table cannot be changed over the air**, so every new device needs the two-slot table flashed by cable before use or it is cable-only for life.
