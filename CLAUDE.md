...
- **OTA:** [`device-firmware-ota.md`](docs/features/device-firmware-ota.md) — Phase 0 (repartition) **shipped** #256; phases 1–4 (manifest + presigned URL, observability, firmware, release workflow) are spec only. **A partition table cannot be changed over the air**, so every new device needs the two-slot table flashed by cable before use or it is cable-only for life.
